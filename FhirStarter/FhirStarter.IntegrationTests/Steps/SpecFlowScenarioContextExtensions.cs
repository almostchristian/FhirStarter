...
namespace FhirStarter.IntegrationTests.Steps
{
    internal static class SpecFlowScenarioContextExtensions
    {
        private static readonly Regex IdVariablePattern = new("({(?<name>\\w+)\\.Id})", RegexOptions.Compiled);
        private static readonly Regex GeneratedRandomVariablePattern = new("({#(?<name>\\w+)(\\((?<generator>.+)\\))?})", RegexOptions.Compiled);
        ...
        {
            var relativeDateTimeRegex = new Regex("([+-])(\\d\\.?\\d*)(d|min|s|h)", RegexOptions.Compiled);
